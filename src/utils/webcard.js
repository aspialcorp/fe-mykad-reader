/******************************************************************************/
// "webcard.js"
// From https://github.com/cardid/webcard
...
;(() => {
  ...
  function Reader(index, name, atr) {
    ...
    self.connect = (shared) => navigator.webcard.send(2, { r: self.index, p: shared ? 2 : 1 })

    self.disconnect = () => navigator.webcard.send(3, { r: self.index })

    self.transceive = (apdu) => navigator.webcard.send(4, { r: self.index, a: apdu })
  }
  ...
  function WebCard() {
    ...
    self.send = (cmdIdx, otherParams) => {
      ...
      return new Promise((resolve, reject) => {
        ...
        self.pendingRequests.set(uid, { c: cmdIdx, resolve: resolve, reject: reject })
    ...
    self.sendEx = (cmdIdx, otherParams) => {
      if (!self.isReady) {
        let promiseWrapper = () => new Promise((_, reject) => reject())
    ...
    self.getVersions = async () => {
      ...
      const timeout = () => new Promise((resolve, _) => setTimeout(() => resolve(), 2000))
      ...
      if (
        typeof pendingVersionCheck !== 'undefined' &&
          ...
          case 1: {
            if (msg.d) {
              ...
              msg.d.forEach((element, index) => {
                _readerList.push(new Reader(index, element.n, element.a))
              })
